...
- in this checkout, start the backend first with `docker compose --env-file .env.development -f compose.yaml -f compose.development.yaml up --build mongo server`
...
- for the migrated app, keep `../.env.development` set to `VITE_API_PROXY_TARGET=http://127.0.0.1:3002`
